...
final class ApplicationService implements IApplicationService
{
    ...
    public function add(string $payload): ResponseModel
    {
        try {
            ...
            $user = $this->userRepository->findBy('userId', $application->userId);
            if (!$user) {
            ...
            $admins = $this->userRepository->findBy('role', Role::ADMIN);
            if (!empty($admins)) {
    ...
    public function findByUserId(int $userId): ResponseModel
    {
        try {
            ...
            return new ResponseModel(200, 'OK', json_encode($this->applicationRepository->findByUserId($userId)));
        } catch (Throwable $e) {
            return new ResponseModel(500, $e->getMessage());
        }
    }
    ...
    {
        try {
            return new ResponseModel(200, 'OK', json_encode($this->applicationRepository->fetchAllActive()));
        } catch (Throwable $e) {
            return new ResponseModel(500, $e->getMessage());
        }
    }
    ...
    public function update(int $applicationId, string $payload): ResponseModel
    {
        try {
            ...
            if ($application->getFromDate() === $fromDate &&
                ...
                return new ResponseModel(202, 'no need to update', json_encode($application));
            }
            ...
            return new ResponseModel(200, 'OK', json_encode($this->applicationRepository->find($applicationId)));
        } catch (Throwable $e) {
            return new ResponseModel(500, $e->getMessage());
        }
    }
    ...
    public function updateStatus(int $applicationId, string $status): ResponseModel
    {
        try {
            ...
            $user = $this->userRepository->findBy('userId', $application->getUserId());
            if (!$user) {
                return new ResponseModel(400, 'user is not valid');
            }

            if ($this->applicationValidator->isStatusValid($status)) {
                return new ResponseModel(400, 'status is not valid');
            }
            ...
            $this->mailService->send(
                ...
                MailTemplate::employeeTemplate($status, $application->getCreatedDatetime()->format('Y/m/d'))
            );

            return new ResponseModel(200, 'OK', json_encode(
